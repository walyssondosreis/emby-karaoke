...
import subprocess  # Para abrir o Explorer no Windows
...
class MainApp:
    def __init__(self, root):
        ...
        self.root.geometry('900x750')  # Aumentei um pouco a altura para acomodar os novos campos
        ...
        self.trilha_audio_var = tk.StringVar(value="instrumental")  # Valor padrão
    ...
    def create_settings_section(self, parent):
        ...
        # Combobox para seleção da trilha de áudio
    ...
    def create_actions_section(self, parent):
        ...
        button_frame4 = tk.Frame(actions_frame, bg=self.colors['light'])  # NOVO: Frame para o botão Gerar Video
        button_frame4.pack(fill=tk.X, pady=5)

        # Todos botões script azul
    ...
    def get_button_style(self, color_type):
        return {'bg': self.colors['primary'], 'fg':'white', 'font': self.fonts['button'], 'relief':'flat','borderwidth':0, 'padx':15, 'pady':8, 'cursor':'hand2'}
    ...
    # NOVO: Função para selecionar pasta stems
    ...
    def gerar_video(self):
        ...
        self.log(f"🚀 Iniciando geração de vídeos...")
        self.log(f"   📁 Pasta Stems: {pasta_stems}")
        self.log(f"   🎵 Trilha de Áudio: {trilha_audio}")
        
        # TODO: Implementar a lógica de geração de vídeos aqui
        # Por enquanto só simulação
        self.log("⚡ Simulação: Processo de geração de vídeos iniciado")
        self.log("   📝 Buscando pastas 'artist - title'...")
        self.log("   🎬 Processando arquivos ultrastar.txt...")
        self.log("   🔊 Usando trilha: " + trilha_audio)
        self.log("✅ Simulação concluída - Aguardando implementação completa")

    # Funções dos scripts originais (mesmo código que você já tinha)
    def verificar(self):
        pasta = self.pasta_var.get(); arquivo = self.arquivo_var.get()
        if not pasta or not arquivo: messagebox.showerror("Erro", "Selecione pasta e arquivo .xlsx."); return
        self.log("🚀 Iniciando verificação de arquivos...")
        try: verificar_arquivos.run(log_callback=self.log, pasta_videos=pasta, caminho_planilha=arquivo)
        except Exception as e: self.log(f"❌ Erro: {str(e)}"); messagebox.showerror("Erro", str(e))

    def gerar_thumbs(self):
        pasta = self.pasta_var.get(); arquivo = self.arquivo_var.get()
        if not pasta: messagebox.showerror("Erro", "Selecione a pasta de vídeos."); return
        self.log("🚀 Iniciando geração de thumbnails...")
        try: gerar_thumb.run(log_callback=self.log, pasta_videos=pasta, arquivo_xlsx=arquivo)
        except Exception as e: self.log(f"❌ Erro: {str(e)}"); messagebox.showerror("Erro", str(e))
    ...
    def normalizar_nomes(self):
        ...
        try: normalizar_nomes.run(log_callback=self.log, pasta_videos=pasta, arquivo_xlsx=arquivo)
        except Exception as e: self.log(f"❌ Erro: {str(e)}"); messagebox.showerror("Erro", str(e))
    ...
    def gerar_nfo(self):
        ...
        try: gerar_nfo.run(log_callback=self.log, pasta_videos=pasta, arquivo_xlsx=arquivo)
        except Exception as e: self.log(f"❌ Erro: {str(e)}"); messagebox.showerror("Erro", str(e))
    ...
    def renomear_arquivos(self):
        ...
        try: renomear_arquivos.run(log_callback=self.log, pasta_videos=pasta, arquivo_xlsx=arquivo)
        except Exception as e: self.log(f"❌ Erro: {str(e)}"); messagebox.showerror("Erro", str(e))
    ...
    def abrir_logs(self):
        # Caminho da raiz do projeto (uma pasta acima de 'app/')
