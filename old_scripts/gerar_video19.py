...
class UltraFastKaraokeGenerator:
    ...
    def calculate_unique_frames(self, lines):
        """Calcula os momentos únicos que precisam de frames diferentes - CORRIGIDO para transições instantâneas"""
        unique_moments = []
        ...
        for i, line_notes in enumerate(lines):
            ...
            # Verificar se há próxima linha e calcular o tempo até ela
            next_line_start = None
            if i + 1 < len(lines):
                next_line_notes = lines[i + 1]
                if next_line_notes:
                    next_line = ' '.join([note['text'] for note in next_line_notes]).strip()
                    next_line_start = self.parser.beat_to_seconds(next_line_notes[0]['beat'])
            
            # CORREÇÃO CRÍTICA: Usar o início da próxima linha como fim, ou adicionar pequena margem
            if next_line_start is not None:
                # Se há próxima linha, terminar exatamente quando ela começa (transição instantânea)
                moment_end = next_line_start
            else:
                # Se é a última linha, adicionar pequena margem
