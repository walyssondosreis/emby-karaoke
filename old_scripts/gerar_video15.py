...
class UltraFastKaraokeGenerator:
    def __init__(self, ultrastar_file, background_image=None, audio_file=None, output_file='karaoke_ultra_fast.mp4', use_gpu=True):
        ...
        # ALTERAÇÃO: Verificar se audio_file foi fornecido via parâmetro
        if audio_file:
...
def main():
    ...
    if args.audio and not os.path.exists(args.audio):
        print(f"❌ Arquivo de áudio não encontrado: {args.audio}")
        sys.exit(1)
