#!/usr/bin/env python3
"""
Gerador de Vídeo Karaokê ULTRA RÁPIDO - Versão Corrigida
"""
...
def get_custom_fonts(project_root):
    ...
    font_files = {
        ...
        'opensans_bold': os.path.join(fonts_dir, "OpenSans-Bold.ttf")
    }
    ...
    if not available_fonts:
        ...
        if system == 'windows':
            available_fonts = {
                'montserrat_bold': r"C:\Windows\Fonts\arialbd.ttf",
                'opensans_bold': r"C:\Windows\Fonts\calibrib.ttf"
            }
            print("   📝 Fallback: Arial Bold e Calibri Bold")
...
# ==================== RENDERIZADOR ULTRA RÁPIDO CORRIGIDO ====================
...
class UltraFastTextRenderer:
    ...
    def load_custom_fonts(self):
        ...
        for size_name, size in sizes.items():
            ...
            try:
                if size_name == 'title' and 'montserrat_bold' in font_files:
                    # Título usa Montserrat-Bold
                    fonts[size_name] = ImageFont.truetype(font_files['montserrat_bold'], size)
                    print(f"   ✅ {size_name}: Montserrat-Bold ({size}px)")
                elif 'opensans_bold' in font_files:
                    # Demais usam OpenSans-Bold
                    fonts[size_name] = ImageFont.truetype(font_files['opensans_bold'], size)
                    print(f"   ✅ {size_name}: OpenSans-Bold ({size}px)")
                else:
                    # Fallback para fonte do sistema
                    if size_name == 'title' and 'montserrat_bold' not in font_files:
                        print(f"   ⚠️  {size_name}: Montserrat-Bold não encontrada, usando fallback")
                    elif 'opensans_bold' not in font_files:
                        print(f"   ⚠️  {size_name}: OpenSans-Bold não encontrada, usando fallback")
...
# ==================== GERADOR ULTRA RÁPIDO CORRIGIDO ====================

class UltraFastKaraokeGenerator:
    def __init__(self, ultrastar_file, background_image=None, output_file='karaoke_ultra_fast_fixed.mp4', use_gpu=True):
    ...
    def generate_video_ultra_fast(self):
        ...
        print(f"🚀 Gerador de Karaokê ULTRA RÁPIDO - Versão Corrigida")
        ...
        try:
            ...
            if os.path.exists(self.output_file):
                ...
                print(f"   🔤 Fontes: Verificação detalhada aplicada")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Gerador de Karaokê ULTRA RÁPIDO - Versão Corrigida')
    parser.add_argument('input', help='Arquivo UltraStar TXT')
    parser.add_argument('--background', '-bg', help='Imagem de fundo')
    parser.add_argument('--output', '-o', default='karaoke_ultra_fast_fixed.mp4', help='Arquivo de saída')
